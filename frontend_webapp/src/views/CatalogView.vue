...
<template>
  <section class="page catalog-page">
    ...
    <p class="catalog-hint">Добавляйте книги в свою коллекцию и отмечайте готовые страницы.</p>
    <transition-group name="catalog" tag="div" class="catalog"><article v-for="book in books" :key="book.id" :class="['catalog-card', { 'is-owned': book.owned }]"><div class="catalog-cover"><img v-if="book.cover" :src="book.cover" :alt="book.title"><div v-else class="cover-placeholder" aria-label="Обложка пока не добавлена">❀</div><span v-if="book.owned" class="collection-badge">В коллекции</span></div><div class="catalog-info"><h2>{{ book.title }}</h2><p>{{ book.author || 'Раскраска' }}</p><div class="catalog-details"><span>{{ book.pages }} работ</span><span v-if="book.owned">{{ book.completed }} готово</span></div><button :class="book.owned ? 'secondary collection-button' : 'primary'" :disabled="busy" @click="book.owned ? removing = book : add(book)"><template v-if="book.owned"><span>В коллекции</span><small>Убрать</small></template><template v-else>Добавить в коллекцию</template></button></div></article></transition-group>
    <div v-if="!books.length" class="empty"><div>⌕</div><h2>Ничего не нашли</h2><p>Попробуйте изменить запрос.</p></div>
    <transition name="modal"><div v-if="removing" class="modal-backdrop" @click.self="removing = null"><section class="confirm-card" role="dialog" aria-modal="true" aria-labelledby="remove-title"><div class="confirm-icon">!</div><h2 id="remove-title">Убрать из коллекции?</h2><p><b>{{ removing.title }}</b> исчезнет из трекера.</p><p v-if="removing.completed" class="warning">Будут удалены {{ removing.completed }} {{ removing.completed === 1 ? 'работа' : 'работы' }} и записи о них в статистике. Это нельзя отменить.</p><div class="confirm-actions"><button class="secondary" :disabled="busy" @click="removing = null">Оставить</button><button class="danger" :disabled="busy" @click="remove">{{ busy ? 'Удаляем…' : 'Удалить' }}</button></div></section></div></transition>
  </section>
</template>
