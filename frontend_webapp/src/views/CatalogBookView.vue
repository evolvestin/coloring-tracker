  ...
  <section v-if="data" class="page catalog-book-view">
    ...
    <div class="catalog-book-intro">
      ...
      <div>
        ...
        <p>{{ data.book.description || 'Описание для этой раскраски пока не добавлено.' }}</p>
        <b>{{ formatCount(data.book.pages, 'page') }}<template v-if="data.book.spreads"> ({{ formatCount(data.book.spreads, 'spread') }})</template></b>
      </div>
    </div>
