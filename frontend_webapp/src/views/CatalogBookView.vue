...
<template>
  <section v-if="data" class="page catalog-book-view">
    ...
    <div class="catalog-book-intro"><img v-if="data.book.cover" :src="data.book.cover" :alt="data.book.title"><div v-else class="cover-placeholder" aria-label="Обложка пока не добавлена">❀</div><div><p v-if="data.book.publisher" class="catalog-publisher">{{ data.book.publisher }}</p><p>{{ data.book.description || 'Описание для этой раскраски пока не добавлено.' }}</p><b>{{ formatCount(data.book.pages, 'work') }}</b></div></div>
