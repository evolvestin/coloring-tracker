...
async function load() {
  loading.value = true
  try {
    const [bookResult, profileResult] = await Promise.all([api('/api/tracker/books/'), api('/api/tracker/profile/')])
...
<template>
  <section class="page tracker-page">
    ...
    <div class="hero-card">
      ...
      <div><b>{{ name }}</b><p>Книг: {{ books.length }} · {{ totals.done }} из {{ totals.total }}</p></div>
    ...
    <p v-if="loading" class="muted">Загружаем коллекцию…</p>
    ...
    <div v-else class="empty"><div>❀</div><h2>Коллекция ждёт первую книгу</h2><p>Выберите раскраску в каталоге — она сразу появится здесь.</p><button class="primary" @click="router.push('/catalog')">Открыть каталог</button></div>
  </section>
</template>
