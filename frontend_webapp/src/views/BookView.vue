...
async function editExisting(target) {
  ...
  const sourceUrl = item?.[sourceKey] || item?.[target]
...
<template>
  <section v-if="data" class="page book-view">
    ...
    <ImageEditor v-if="editor" :file="editor.file" :title="editor.target === 'cover' ? 'Обложка раскраски' : editor.target === 'photo' ? 'Фото работы' : 'Цветовой код'" @cancel="editor = null" @save="saveEditedImage">
    </ImageEditor>
