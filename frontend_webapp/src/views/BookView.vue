...
async function uploadColorCode(event) {
  const file = event.target.files[0]
  if (!file || !activePage.value) return
  ...
  try {
    ...
    await api(`/api/tracker/books/${route.params.id}/pages/${activePage.value.id}/color-code/`, { method: 'POST', body: form })
    await load()
  } finally {
    uploading.value = ''
    event.target.value = ''
