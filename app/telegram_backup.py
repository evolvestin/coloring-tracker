...
MANIFEST_FORMAT = 'assethub-telegram-backup-v2'


def _file_id_caption(file_id: str) -> str:
    return f'<code>{escape(file_id)}</code>'
...
class TelegramBotApi:
    ...
    def _call(self, method: str, *, data=None, files=None):
        try:
            ...
            raise TelegramBackupError(
                f'Telegram {method} request failed: {self._safe_error(error.__class__.__name__)}'
            ) from error
        try:
        ...
        if not isinstance(payload, dict) or not payload.get('ok') or 'result' not in payload:
            description = (
            ...
            raise TelegramBackupError(
                ...
                f'{self._safe_error(description)}'
            )
    ...
    def probe(self) -> dict:
        ...
        if not chat_id:
            raise TelegramBackupError('Telegram backup channel ID is not configured')
        result = self._call('getChat', data={'chat_id': str(chat_id)})
    ...
    def send_document(self, path: str, *, filename: str, caption: str) -> TelegramDocument:
        ...
        if settings.TELEGRAM_BACKUP_LOCAL_FILE_MODE:
            ...
            if os.path.getsize(path) > 50 * 1024 * 1024:
                raise TelegramBackupError(
                    'Large Telegram files require TELEGRAM_BACKUP_LOCAL_FILE_MODE=1 '
                    'and the local Bot API server'
                )
            with open(path, 'rb') as source:
                result = self._call(
                    'sendDocument',
        ...
        *,
        ...
        try:
            for index, (path, filename) in enumerate(zip(paths, filenames, strict=True)):
                ...
                elif not settings.TELEGRAM_BACKUP_LOCAL_FILE_MODE:
                    if os.path.getsize(path) > 50 * 1024 * 1024:
                        raise TelegramBackupError(
                            'Large Telegram files require TELEGRAM_BACKUP_LOCAL_FILE_MODE=1 '
                            'and the local Bot API server'
    ...
    def download_file(self, file_id: str, destination: str | os.PathLike[str]) -> None:
        ...
        if settings.TELEGRAM_BACKUP_LOCAL_FILE_MODE and os.path.isabs(file_path):
            try:
                ...
                raise TelegramBackupError(f'Cannot copy local Telegram file: {error}') from error
        ...
        try:
            ...
            raise TelegramBackupError(
                f'Telegram file download failed: {self._safe_error(error)}'
            ) from error
...
def _final_groups(items: list[dict]) -> list[list[dict]]:
    groups = [items[start : start + 10] for start in range(0, len(items), 10)]
...
def upload_backup(path: str, *, source_filename: str | None = None) -> TelegramBackup:
    ...
    backup = TelegramBackup.objects.create(
        ...
        sha256=sha256_file(path) if size_bytes else '',
    )
    ...
    try:
        ...
        for _number, part_path, _part_size, _part_hash, filename in part_metadata:
            document = client.send_document(part_path, filename=filename, caption='')
        ...
        manifest = {
            ...
            'parts': [
                ...
                for (number, _part_path, part_size, part_hash, filename), staged_document in zip(
                    part_metadata, staged_documents, strict=True
                )
            ],
        }
        ...
        for group in _final_groups(final_items):
            ...
            for item, document in zip(group, group_documents, strict=True):
        ...
        client.edit_message_caption(
            ...
            _file_id_caption(manifest_document.file_id),
        )
        ...
        backup.save(
            update_fields=('manifest_message_id', 'manifest_file_id', 'status', 'updated_at')
        )
        return backup
        ...
        backup.error_message = error_message[-2000:]
...
def load_manifest(path: str | os.PathLike[str]) -> dict:
    ...
    try:
        ...
        if not isinstance(backup_id, str):
            raise ValueError
        UUID(backup_id)
        total_size = _strict_positive_integer(manifest['size_bytes'])
    ...
    for expected_number, part in enumerate(manifest['parts'], start=1):
        ...
        try:
            number = _strict_positive_integer(part['part_number'])
            size = _strict_positive_integer(part['size_bytes'])
        except (KeyError, TypeError, ValueError) as error:
            raise TelegramBackupError('Telegram manifest contains invalid part metadata') from error
        if (
...
def download_backup(backup: TelegramBackup, destination: str) -> None:
    ...
    try:
        ...
        if (
            os.path.getsize(destination) != backup.size_bytes
            or sha256_file(destination) != backup.sha256
        ):
